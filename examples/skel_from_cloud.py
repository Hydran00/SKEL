...
class SKELModelOptimizer:
    def __init__(self, skel_model, device=DEVICE):
        ...
        self.betas = torch.zeros((1, NUM_BETAS), dtype=torch.float32, requires_grad=True, device=DEVICE)
        self.poses = torch.zeros((1, self.skel_model.num_q_params), dtype=torch.float32, requires_grad=True, device=DEVICE)
        self.global_orient = torch.zeros((1, 3), dtype=torch.float32, requires_grad=True, device=DEVICE)
        self.global_position = torch.zeros((1, 3), dtype=torch.float32, requires_grad=True, device=DEVICE)
    ...
    def optimize_model(self, target_pc):
        ...
        # Optimization steps
        self.optimize([self.global_position], lr=0.05, loss_type="transl", num_iterations=200, tolerance_change=1e-5)
        self.optimize([self.poses], lr=0.01, loss_type="rot", num_iterations=200, tolerance_change=1e-4)
        self.optimize([self.poses], lr=0.02, loss_type="transl", num_iterations=200, tolerance_change=1e-4)
    ...
    def point_cloud_to_tensor(self, point_cloud):
        return torch.tensor(np.asarray(point_cloud.points), dtype=torch.float32) if isinstance(point_cloud, o3d.geometry.PointCloud) else torch.tensor(point_cloud, dtype=torch.float32)

    def compute_loss(self, loss_type, generated_pc):
        return self.chamfer_distance(generated_pc, self.target_pc_tensor.unsqueeze(0), reverse=True)
    ...
    def optimize(self, params, lr, loss_type, num_iterations, tolerance_change):
        ...
        for i in pbar:
            ...
            self.update_visualization(skin_vertices.cpu().detach().numpy())
    ...
    def init_visualization(self, target_pc):
        ...
        ref_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.6, origin=[0, 0, 0])
...
def send_optimized_results(data):
    socket_send.send(pickle.dumps(data))
...
def main():
    ...
    while True:
        ...
        send_optimized_results(results)
